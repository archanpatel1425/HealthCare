...
const ConsultedPatients = () => {
  ...
  useEffect(() => {
    axios
      .post(`${import.meta.env.VITE_API_URL}/doctor/done`, { doctorId: patientData?.doctorId })
      .then((res) => {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold text-gray-700">Consulted Patients</h1>
          ...
          <table className="min-w-full border-collapse">
            ...
            <tbody>
              {filterPatients.map((patient, index) => (
                <tr key={index} className="hover:bg-green-50 cursor-pointer" onClick={() => handleRowClick(patient)}>
                  ...
                  <td className="px-6 py-3 border-b">
                    {new Date(patient.date).toLocaleDateString("en-GB").replace(/\//g, "-")} - {patient.time}
                  </td>
                </tr>
              ))}
            </tbody>
