...
const SkinCancer = () => {
  ...
  const fetchPrecautionsFromGemini = async (disease) => {
    ...
    try {
      ...
      const precautionsList = text.match(/^[*\-•]\s*(.+)/gm)?.map(item => item.replace(/^[*\-•]\s*/, ""));

      if (precautionsList) {
        setPrecautions(precautionsList.join("\n"));
      } else {
        setPrecautions("No structured precautions found. Try refining the prompt.");
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto bg-white shadow-lg rounded-xl border border-gray-200 flex flex-col lg:flex-row gap-4 min-h-[450px] h-full w-full">
      ...
      <div className="flex flex-col items-center w-full lg:w-1/2 flex-1">
        <h1 className="text-3xl font-bold text-center text-green-600 mb-4">Skin Cancer Assistant</h1>
        <h1 className="text-sm font-bold text-center text-green-600 mb-4">Caution : (Prediction Might Be Wrong Don't Rely On It.)</h1>
        <input
          type="file"
          accept="image/*"
